			...
			if (count($sought_in_arr) > $urls_count){
				...
				if (isset($blinkr_email) and isset($_POST["for_cron"])) {
					...
					foreach ($sought_in_arr as $k => $url_list) {
						...
						$query = "INSERT INTO blinkr_check_links VALUES ('', '". $blinkr_email ."', '".$sought_for_arr."', '".$url_list."', '".$parse."', 0, 0, '" . $time_of_add . "', 0 )";
                        if (!$wpdb->query($query)) {
    						echo "insert error!!!";
    						exit;
    					}
				...
				foreach($sought_in_arr as $key => $sought_in_url) {
					...
					if ($url_result or ($text and $_POST["dont_recheck"])) {
                        ...
                        $result .= $sought_in_url.",Has no URLs\r\n";


                    }
				}

//                $result = blinkr_writeToFile($file_name.time().".csv", $result);
...
<html>
	<head>
		<title>Check Backlinks URLs - Rechecker</title>
	</head>
	<body>
        ...
        <div id="message" class="updated">
            <p>
                ...
                Remember, once you have your rechecked your backlinks, increase the power of them as outlined in the <a href="http://www.expertteam.tv" title="Cost effective staff and training to run it all for you">ExpertTeam.tv system</a>
            </p>
        </div>
        <?php
        endif;
		...
		<table border="0" cellpadding="5" cellspacing="5" align="left">
			<tr>
				<td>
					<h1>Check Backlink URLs - Rechecker</h1>
Check if your backlinks are live - automatically part of the <a target="_blank" href="http://www.expertteam.tv">ExpertTeam.tv system (opens in a new window)</a>.  				</td>
			</tr>
			<?	if (isset($message)) {  ?>
				<tr>
